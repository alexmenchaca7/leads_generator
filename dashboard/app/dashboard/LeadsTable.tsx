  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2 rounded-xl border border-slate-800 bg-slate-900 p-3">
        {/* Acciones: 3 botones iguales en móvil, en línea en desktop */}
        <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">
          <button
            onClick={() => setEditState({ mode: "create", lead: null })}
            className="flex items-center justify-center rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white hover:bg-indigo-500"
          >
            + Nuevo
          </button>
          <button
            onClick={() => setShowBlocklist(true)}
            className="flex items-center justify-center rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-300 hover:bg-slate-800"
          >
        ...
        <div className="flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 outline-none focus:border-indigo-500"
          />
          <button
      ...
      {selected.size > 0 && (
        <div className="space-y-2 rounded-xl border border-indigo-500/40 bg-indigo-500/10 p-3 text-sm sm:flex sm:flex-wrap sm:items-center sm:gap-2 sm:space-y-0">
          <span className="block font-medium text-indigo-200">
            {selected.size} seleccionado{selected.size !== 1 ? "s" : ""}
          </span>
          {selected.size < sorted.length && (
            <button
              onClick={selectAllFiltered}
              className="w-full rounded-lg border border-indigo-500/40 px-3 py-2 text-indigo-200 hover:bg-indigo-500/20 sm:w-auto"
            >
